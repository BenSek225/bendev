...
export default function ProjectsPage() {
  ...
  return (
    <main className="bg-primary text-text-primary">
      <section className="relative overflow-hidden border-b border-white/10 pb-20 pt-40 md:pb-28 md:pt-52">
        <div className="hero-noise pointer-events-none absolute inset-0 opacity-30" aria-hidden="true" />
        <div className="container-portfolio relative"><SectionLabel index="01">Projets</SectionLabel><motion.div variants={fadeUp} initial="hidden" animate="visible"><h1 className="mt-10 max-w-4xl font-serif text-hero-sm leading-[1.08] text-balance md:text-hero-md lg:text-hero">Des projets réalisés pour avoir un <em className="text-accent">impact.</em></h1><p className="mt-8 max-w-xl text-body-lg leading-relaxed text-secondary">Une sélection de mes réalisations récentes en développement web, applications mobiles et solutions fullstack.</p></motion.div></div>
      </section>
      <section className="section-spacing"><div className="container-portfolio"><div className="flex flex-wrap items-center justify-between gap-6 border-b border-white/10 pb-6"><p className="font-mono text-label uppercase text-secondary">Filtrer la sélection</p><div role="tablist" aria-label="Filtrer les projets" className="flex flex-wrap gap-2">{filters.map((filter) => <button key={filter} type="button" role="tab" aria-selected={activeFilter === filter} onClick={() => setActiveFilter(filter)} className={`rounded-full border px-4 py-3 font-mono text-label uppercase transition-colors ${activeFilter === filter ? 'border-accent bg-accent text-primary' : 'border-white/15 text-secondary hover:border-accent hover:text-text-primary'}`}>{filter}</button>)}</div></div><motion.div layout variants={staggerContainer} initial="hidden" animate="visible" className="mt-12 grid gap-x-8 gap-y-14 md:grid-cols-2 lg:grid-cols-3"><AnimatePresence mode="popLayout">{visibleProjects.map((project) => <ProjectCard key={project.slug} project={project} />)}</AnimatePresence></motion.div></div></section>
      <section className="relative isolate overflow-hidden border-t border-white/10 py-24 md:py-32"><Image src="/portfolio/contact.png" alt="Espace architectural sombre pour démarrer un projet" fill sizes="100vw" className="img-warm absolute inset-0 -z-20 object-cover" /><div className="absolute inset-0 -z-10 bg-primary/80" aria-hidden="true" /><div className="hero-noise pointer-events-none absolute inset-0 -z-10 opacity-20" aria-hidden="true" /><div className="container-portfolio relative flex flex-col items-start gap-8 md:flex-row md:items-center md:justify-between"><h2 className="max-w-2xl font-serif text-h2-sm text-balance md:text-h2">Vous avez un projet en tête&nbsp;?</h2><CTAButton label="Discutons de votre projet" href="/contact" /></div></section>
      <Footer />
    </main>
  )
}
